...
static void insertNode(TreeNode *t)
{
    ...
    case statementK:
      switch (t->kind.stmt){
        case variableK:
        // printf("TESTE: nome=%s, escopo=%s, lookup atual=%d / %d\n", t->attr.name, t->attr.scope, st_lookup(t->attr.name, t->attr.scope), st_lookup(t->attr.name, "global"));
        ...
        case callK:
          ...
          st_lookup(t->attr.name, "input") != -1 &&
          st_lookup(t->attr.name, "output") != -1
          ){
...
static void checkNode(TreeNode *t)
{
	...
	case statementK:
        ...
        if ((strcmp(findFunType(t->child[1]->attr.name, "global"), "void")) == 0){
